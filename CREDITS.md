## Copyright & Licenses

This project makes use of the following third-party software:

**FFmpeg**
Copyright (c) 2000-2026 the FFmpeg developers.
This project uses `ffmpeg.exe` as an unmodified, separate executable,
invoked as an external process (via command-line, not linked into this
application's binary). The specific FFmpeg build in use is licensed
under GPLv3 (https://ffmpeg.org/legal.html).
The FFmpeg source code and license text are available at:
https://ffmpeg.org

**Qt**
Copyright (c) The Qt Company Ltd. and other contributors.
Qt is available under the GNU Lesser General Public License (LGPL) version 3,
and other licensing terms.
https://www.qt.io

**CMake**
Copyright (c) 2000-2026 Kitware, Inc. and Contributors.
Distributed under the BSD 3-Clause License.
https://cmake.org

**Inno Setup**
Copyright (c) 1997-2026 Jordan Russell. Portions Copyright (c) 2000-2026 Martijn Laan.
All rights reserved. Used under the Inno Setup License.
https://jrsoftware.org/isinfo.php

**whisper.cpp**
Copyright (c) 2023-2026 Georgi Gerganov.
Distributed under the MIT License.
https://github.com/ggerganov/whisper.cpp

Full license texts for third-party components are available in the
`third_party_licenses/` directory of this repository.

---

All trademarks, service marks, and trade names used in this document are the
property of their respective owners. This project is not affiliated with or
endorsed by any of the above projects unless explicitly stated.